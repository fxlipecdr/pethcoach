...
## Current phase
Prompt Mestre + P0 + P1 + P2 + P3 delivered and technically validated. P2 uses a development Supabase project with Resend SMTP; migrations, remote SQL RLS tests, anonymous denial, real PKCE/session/logout/replay, two-user app isolation and direct two-JWT PostgREST acceptance passed. The production Vercel callback/login was also confirmed by the responsible user. Supabase CLI-generated local types are committed separately from the grant/check-constrained app contract; natural JWT expiry, valid/invalid refresh and controlled network failure passed against the disposable local Auth/Data API stack. P3 adds three unique problem landings, reward-based starter practices, deterministic referral copy, per-page metadata/canonical/OG and a ready-pages-only sitemap. Global and page-level noindex remain in force until editorial/professional approval. The official PethCoach logo and palette are defined in `docs/ui-design.md`; do not replace or reinterpret them without an explicit product decision. The next implementation phase is P4. The Vercel Hobby validation deploy at `https://coach.peth.com.br` has valid DNS/HTTPS and production-only public Supabase envs; it remains noindex and is not a customer release. See `docs/external-services.md`, `docs/p2-acceptance.md`, `docs/p2-setup.md` and `docs/p3-acceptance.md`. Layout/form previews are development-only presentation fixtures, never an auth bypass. No live coaching, checkout or AI generation is enabled. Never equate email delivery, PGlite, SQL impersonation or mocked-provider tests with real Auth/PostgREST validation.
