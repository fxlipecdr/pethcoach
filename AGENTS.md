...
## Current phase
Prompt Mestre + P0 + P1 delivered and validated. P2 is connected to a development Supabase project with Resend SMTP; migrations, remote SQL RLS tests and anonymous Data API denial passed. Real email/PKCE and two-user JWT/PostgREST acceptance remain pending. See `docs/external-services.md`, `docs/p2-acceptance.md` and `docs/p2-setup.md` before declaring P2 complete or proceeding to P3. Colors and logo remain provisional and configurable. Layout/form previews are development-only presentation fixtures, never an auth bypass. Auth and dog CRUD require configured Supabase; no live coaching, checkout or AI generation is enabled. Never equate PGlite, SQL impersonation or mocked-provider tests with real Auth/PostgREST validation.
