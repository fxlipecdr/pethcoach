...
Por decisão do responsável, o projeto permanece no Hobby apenas durante a validação sem vendas. Em 01/09/2026, o deployment P5 `dpl_Axtt1ye8exZjL3ZcCPE3ZP1EppFH`, do commit `946406e`, ficou `READY` e foi associado a `https://coach.peth.com.br`. As quatro variáveis de runtime estão limitadas a Production; Preview continua sem acesso ao projeto Supabase dev. A verificação pública retornou home 200, criação 201, dez respostas, conclusão `BLOCK` e resultado 200; `robots.txt` manteve `Disallow: /` e os dados/rate limits descartáveis foram removidos. Auth continua habilitado e `/app` redireciona para login sem sessão. A aplicação fecha `/dev/*` em produção e continua sem Analytics ou Speed Insights. Migrar para Pro antes da operação comercial; o domínio, Auth e quiz ativos não representam liberação para clientes.

P5 foi concluída tecnicamente no repositório com gate determinístico antes de qualquer futura IA, três desfechos, auditoria versionada e mensagens fixas de segurança. A aplicação mantém `Disallow: /` e `noindex`; revisão editorial/profissional continua obrigatória antes de indexar.
